...
# 查询昨日(2026-06-05)发布的视频
cur.execute("SELECT * FROM video_metrics WHERE DATE(publish_time) = '2026-06-05' ORDER BY platform, publish_time")
videos = [dict(r) for r in cur.fetchall()]

# 查询今日异常告警
cur.execute("SELECT * FROM anomaly_alerts WHERE DATE(created_time) >= '2026-06-06' ORDER BY platform")
alerts = [dict(r) for r in cur.fetchall()]

conn.close()

print(f'昨日视频: {len(videos)}条')
print(f'今日异常: {len(alerts)}条')
print()
...
print("=" * 80)
print("【一】视频数据全览")
print(f"{'平台':<6} {'视频标题':<28} {'播放量':>8} {'完播率':>7} {'点赞率':>7} {'评论':>6} {'推流判定':>10}")
print("-" * 85)
for v in videos:
    pf = "抖音" if v['platform'] == 'douyin' else "小红书"
    title = v['video_title'][:26]
    qualify = (v['completion_rate'] > COMPLETION_THRESHOLD) and (v['like_rate'] > LIKE_RATE_THRESHOLD)
    tag = "✅ 建议投流" if qualify else "❌ 待优化"
    print(f"{pf:<6} {title:<28} {v['play_count']:>8,} {v['completion_rate']:>6.1%} {v['like_rate']:>6.1%} {v['comment_count']:>6} {tag:>10}")

print(f"\n{'=' * 80}")
print(f"【二】推流指数评估（条件：完播率>30% 且 点赞率>3%）")
print(f"{'=' * 80}")
...
for a in alerts:
    ...
    print(f"     实际：{a['actual_value']} | 基准：{a['benchmark_value']}")
...
print(f"{'=' * 80}")
print(f"【四】昨日整体运营数据汇总（2026-06-05）")
print(f"{'=' * 80}")
...
# ============ 输出JSON ============
result = {
    "monitor_date": "2026-06-05",
    "fetch_time": "2026-06-06 21:36",
