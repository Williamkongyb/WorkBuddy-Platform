...
# 检查是否已有6月3日的数据
cursor.execute("SELECT COUNT(*) FROM videos WHERE publish_time LIKE '2026-06-03%'")
if cursor.fetchone()[0] > 0:
    print("[INFO] 6月3日数据已存在，跳过插入")
    conn.close()
    exit()
...
conn.commit()
conn.close()
print(f"[OK] 成功插入 {len(new_videos)} 条视频 + {len(new_metrics)} 条metrics数据（6月3日）")
